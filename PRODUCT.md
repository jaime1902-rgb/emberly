...
The earlier broad "decided-on-AI, comparing vendors" audience still describes organ-IA's eventual general positioning once past this launch campaign; record both, current campaign wins for now.

## Product Purpose

organ-IA is an AI automation studio. The site's only job is lead capture. Current build is a single, specific campaign: **la oferta de las 3 plazas** — organ-IA is selecting 3 clinics for a free 30-day pilot of its WhatsApp booking assistant, in exchange for real, documented results (case studies). The funnel sells this specific offer and qualifies applicants for it; it is not general informational content.

## Positioning

Launch-phase positioning: organ-IA just launched and has the technology but no local case studies yet. The 3-clinic pilot is an explicit, honest trade — free implementation for the clinic, a real documented case study for organ-IA. Longer-term positioning (once past launch): deep technical specialization in AI/automation, custom-built agents and complex integrations rather than templated setups.

## Operating Context

Visitor is a clinic owner/manager evaluating whether to apply for one of the 3 free pilot spots. Current funnel (5 screens, one question per screen, inside a single card — see DESIGN.md): hero (terminal-style scarcity counter) → clinic type → pain point (with a fade-in follow-up on message volume) → contact details (nombre, clínica, teléfono WhatsApp, ciudad — no email field) → confirmation with a WhatsApp deep link. "Solicitar mi plaza" is a reviewed application, not an instant booking — organ-IA confirms fit within 24h. The standalone "offer explainer" screen (qué es / qué recibe / por qué gratis / por qué solo 3 / el candado) that an earlier build had was dropped from the flow at the user's explicit direction; that trust content now lives only in the hero subhead and the legal line under the contact form.

## The 3-Spots Offer — durable facts (do not alter without the user)

- **What:** organ-IA implements its automation assistant free for 3 clinics for 30 days. Works from day 1. Clinic pays nothing during the pilot.
...
- **Why it's free:** organ-IA just launched — has the tech, lacks real local case studies. The trade: clinic gets the service free, organ-IA gets a real, measurable, documented case study (appointments booked, leads answered, no-shows reduced).
...
- **After the 30 days:** organ-IA presents results (messages answered, appointments booked, recovered leads). Clinic then decides whether to continue on the paid monthly plan — no pressure, no fine print.
...
- The confirmation screen's WhatsApp button links to a placeholder number (`ORGANIA_WHATSAPP_NUMBER` in `components/organia/funnel.tsx`) — **must be replaced with organ-IA's real WhatsApp Business number before launch.**
...
- Name: **organ-IA**, subtitle **AI Automation Studio**. Tagline: "Automatizamos procesos. Multiplicamos resultados."
- Mark: the studio's real horse-head logo asset (black, circuit-trace mane), extracted with a transparent background at `public/organia-mark.png`; the full lockup with wordmark lives at `public/organia-logo.png`. Not a redrawn approximation — use the real asset for the mark everywhere.
